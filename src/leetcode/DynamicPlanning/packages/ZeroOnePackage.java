 ...
 *   如果使用顺序遍历则表示 max{dp[V], dp[V-Ci]+Wi} 取的是 max{dp[i-1][V], dp[i][V-Ci]+Wi}
 *
 * ==============================================================================================================================
 * 问题变种
 * 有 N 件物品和一个容量为 V 的背包。放入第 i 件物品耗费的费用是 Ci，得到的 价值是 Wi。求解将哪些物品装入背包且恰好装满背包可使价值总和最大。
 * 这里和原题的区别在于恰好装满背包, 跟初始化的状态有关，因为如果子问题满足条件，最终解才会满足条件
 ...
 *
 * 区别初始化： 适用于dp[N+1][V+1]多申请一位的情况
 * 对于装满背包求最大值  初始化 dp[0]=0 , dp[1...N]=负无穷
 * 对于装满背包求最小值  初始化 dp[0]=0 , dp[1...N]=正无穷
 * 对于无需装满背包的情况   dp[0..N]=0
 *
 * 如果dp[N][V+1] , 在手动判断第一行和第一列的情况， 如果要求恰好装满情况，则第一行的值取第一个物品体积恰好等于V的价值，其余为最小值
 *                                                                  第一列为最小值
 *
 *
 * @Author: chaoyi.zhang
...
//        int[] w = new int[]{3,4,5,7};
//        int[][] dp = zop.getDp3(c, w, 9);
